 ...
 * </ul>
 */
class MultinomialNaiveBayes extends OnlineClassifier {
...
/* Train the model incrementally with a single instance given for training.
 *
 * @param input an instance
 * @return Unit
 */
  override def trainIncremental(input: Example): Unit = {
    val tmodel = new MultinomialNaiveBayesModel(model.numClasses, model.numFeatures,
   ...
   */
  def predictSingle(input: Example): (Example, Double) = (input, model.predict(input))
